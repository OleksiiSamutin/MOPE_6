...
while True:
    ...
    # формування Y
    y = [
        [round(5.4 + 8.1 * x[0][j] + 7.5 * x[1][j] + 9.7 * x[2][j] + 5.2 * x[0][j] * x[0][j] + 0.6 * x[1][j] * x[1][j] +
               4.7 * x[2][j] * x[2][j] + 8.1 * x[0][j] * x[1][j] + 0.8 * x[0][j] * x[2][j] + 7.4 * x[1][j] * x[2][j] +
